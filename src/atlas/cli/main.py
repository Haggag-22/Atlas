...
def plan(
    ...
    async def _run() -> None:
        ...
        # Ensure pathfinding.cloud attack knowledge is ready (syncs on first run)
        from atlas.knowledge.pathfinding_loader import _PATHFINDING_DIR, sync_pathfinding_data
        _needs_sync = not (_PATHFINDING_DIR.exists() and list(_PATHFINDING_DIR.glob("*.yaml")))
        if _needs_sync:
            with console.status("[dim]Preparing attack knowledge (pathfinding.cloud)...[/dim]"):
                try:
                    sync_pathfinding_data()
                except Exception:
                    pass  # Plan continues; pathfinding edges may be limited
